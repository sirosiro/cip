...
1.  **Pure-Intent Mode (完全回避・長期戦略用)**
    ...
    *   **CIP-Scriber使用義務: 【必須（Obligation）】**。マニフェストから直接Python等の実装コードを生成することを固く禁ずる。必ず中間言語である `CIP-Scriber` を経由して論理（Intent）を抽象化し、そのScriberコードを検証した上で最終的なソースコードに変換しなければならない。
...
3.  **Agility-First Mode (初期案・PoC用)**
    ...
    *   **CIP-Scriber使用義務: 【非推奨（Bypass）】**。中間言語による抽象化プロセスをスキップし、要件定義から直接Python等の実行可能コードを最速で生成することを優先する。
