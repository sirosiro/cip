...
*   **CIP-Scriber (Mermaid) 出力の永続化義務**: CIP-Scriberを用いてMermaidコードを生成する際は、ターミナル（画面）への出力だけで済ませてはならない。必ず `write_file` 等を用いて、生成したMermaidコードを `対象機能名_flow.mmd` のような `.mmd` 拡張子を持つファイルとして、**対象となるソースコードと同じディレクトリ（各モジュールのディレクトリ内）**に保存し、後工程で参照可能な資産として永続化すること。
...
1.  **Pure-Intent Mode (完全回避・長期戦略用)**
    ...
    *   **CIP-Scriber使用義務: 【必須（Obligation）】**。マニフェストから直接Python等の実装コードを生成することを固く禁ずる。必ず視覚的中間言語（Mermaidベース）である `CIP-Scriber` を経由して論理（Intent）を抽象化し、そのScriberコード（図とアノテーション）を検証した上で最終的なソースコードに変換しなければならない。
        *   **【仕様書の遅延読み込み】** AIは事前に `CIP-Scriber` の仕様を熟知していない場合、仕様書ファイルを自律的に探索して `read_file` ツール等で読み込むこと。
        *   探索パスの優先順位：① `_docs/standards/CIP_Scriber.md` (CIP-Bridge稼働時)、② `./CIP_Scriber.md` (手動配置時)、③ プロジェクトルートの `docs/standards/CIP_Scriber.md`。
2.  **Hybrid-Bridge Mode (中間・実用標準)**
    ...
    *   **CIP-Scriber使用義務: 【任意（Optional）】**。複雑なコアロジックを設計する際はScriberの利用を推奨するが、単純な機能や既存ライブラリのグルーコード（繋ぎ込み）等であれば、直接実装コードを生成してもよい。
3.  **Agility-First Mode (初期案・PoC用)**
    ...
    *   **CIP-Scriber使用義務: 【非推奨（Bypass）】**。視覚的中間言語による抽象化プロセスをスキップし、要件定義から直接Python等の実行可能コードを最速で生成することを優先する。
