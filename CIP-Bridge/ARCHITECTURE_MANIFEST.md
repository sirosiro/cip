...
*   **Date:** 2026-02-23
*   **Decision:** AIの思考プロセスによる誤検知を防ぐため、パース前に `<thinking>` ブロックを完全に除去。
*   **Decision:** 引用ループを防ぎつつ自己発言を許可するため、`[FROM: @...]` が自分のIDと一致する場合に限り転送を許可（自己署名プロトコル）。
