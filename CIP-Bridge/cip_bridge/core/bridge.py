...
# @intent:responsibility PTY (Pseudo-Terminal) の制御、外部プロセスの実行、各レイヤーコンポーネントの統合、
#                         およびシグナル駆動のイベントループを管理する。
class BridgeCore:
    def __init__(self, cmd: List[str] = None, bus_id: str = None, base_dir: str = "./cip_bus"):
        ...
        # @intent:rationale 疎結合な 4階層モデルを採用し、将来のトランスポート層（Redis等）の拡張を容易にする。
        ...
        # @intent:responsibility シグナルハンドラからメインループへ安全に通知するための自己パイプ。
    ...
    def log_event(self, msg: str):
        ...
        try:
            with open("bridge_events.log", "a") as f:
                f.write(f"[{timestamp}] [BridgeCore:{self.bus_id}] {msg}\n")
        except Exception: pass

    # @intent:responsibility 親ディレクトリを遡り、共通の設計思想（DESIGN_PHILOSOPHY.md）をリンクする。
    ...
    # @intent:responsibility Inboxから受信したメッセージを処理し、Pull型通信用のファイルを作成し、
    #                         AIに読み込みを促すシステム通知を PTY に注入する。
    def process_inbox(self):
        ...
        from ..utils.text_utils import remove_ui_noise
    ...
    # @intent:responsibility 起動時にAIに対してCIPエコシステムの役割を認識させ、初期化を促す。
            ...
            while True:
                ...
                if self.master_fd in rfds:
                    ...
                    if not self.bootstrapped:
                        from ..utils.text_utils import strip_ansi, remove_thinking_block
                    ...
                    # @intent:responsibility 出力ストリームを解析し、CIPタグがあれば交渉相手へのルーティングを行う。
