# CIP-Scrivener (Constraint-Injected Principles Scrivener) 言語仕様書 Version 2.0 - The Visual Manifest

設計者が掲げる意図にまつわる原則（Principles）を、AIが逸脱不可能な物理的制約（Constraint）としてフローチャートにアノテーションで直接注入する記法、制約注入型原則書記官 CIP-Scrivener です。

---

## 0. CIP-Scrivener（略称：スクリバー）
本ツールの名称は、構想当初は「筆記者」を意味する Scriber でしたが美容のスクライバーと近接するのを避けるため、現在では古語で「代書人」を意味する Scrivener に由来します。Scrivenerの 正確な発音は「スクリヴナー」に近いものですが発音しづらいため、本プロジェクトにおいては **独自のアイデンティティとして、あえて親しみを込めて「スクリバー」** と呼称します。
...
CIP-Scrivener 、Mermaidを基盤とし、人間とAIが「意図」を共有することで、実コードへの安全な橋渡しを行う。
...
CIP-Scrivener は、Mermaidの標準構文の中に、独自のメタデータ（アノテーション）をMarkdown形式で埋め込む。
...
実コードでは冗長な構文に論理が埋没する。CIP-Scrivener はこれらを視覚的なノードに分離することで、レビュー者が「言語の書き方」ではなく「設計の妥当性」の確認に集中できる環境を提供する。
