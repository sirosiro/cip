...
* **意図駆動型ワークフロー**
    コードを書く前に必ずこれらのマニフェストを更新・参照し、AIに「設計者の人格と意図」を憑依させてから実装を行う開発フローを実践しています。
...
* **フラクタル・アーキテクチャ (Fractal Placement)**
    大規模プロジェクトに対応するため、マニフェストを再帰的に配置する手法です。システム全体のマニフェストの下に、サブシステムやモジュールごとのマニフェストを階層的に配置し、複雑なドメインでも一貫した設計思想を隅々まで浸透させることを目指します。
* **Linter for Intent**
