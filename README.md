...
本プロジェクトは、人間とAIが「設計意図（Intent）」を核に協力し、高品質で持続可能なソフトウェアを開発するためのフレームワークです。ここではこの手法を **Core-Intent Prompting (CIP)** と呼びます。このプロジェクトの方針は、数多くある生成AIサブスクリプションのうち無料の利用範囲のみでどこまで実用的なコードを生成できるのかを模索するとともに資金やAI知識弱者同士が協力してどうやって成果出すのかその手法を開拓することを目指して開発しています。全く個人の趣味で作成している環境のため動作保証はできかねますのでご了承ください。
